...
class Player
  ...
  addTrack: (track, buttIn=false) ->
    @updaterPlaylist.addTracks(spotify.createFromLink(track))
...
module.exports = (robot) ->
  ...
  player.init('user', 'pass', ->
    ...
    robot.respond /spotify (play|add) (.*)/i, (msg) ->
      player[msg.match[1]].call(player, msg.match[2])
    ...
    robot.respond /spotify show playlist/i, (msg) ->
      reply = ""
      for tracks in play.getPlaylistTracks()
        reply += "#{displayTrack(track)}\n"
